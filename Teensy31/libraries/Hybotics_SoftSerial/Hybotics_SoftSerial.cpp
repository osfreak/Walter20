...
Hybotics_SoftSerial::Hybotics_SoftSerial(uint8_t rxPin, uint8_t txPin, bool inverse_logic /* = false */) {
...
void Hybotics_SoftSerial::begin(unsigned long speed) {
	if (port) {
		port->begin(speed);
	} else {
...
size_t Hybotics_SoftSerial::write(uint8_t b) {
...
void Hybotics_SoftSerial::flush() {
	if (port) {
		port->flush();
	}
}
...
int Hybotics_SoftSerial::available() {
	int nrBytes;

	if (port) {
		nrBytes port->available();
	} else {
		nrBytes = 0;
	}
...
// Statics
//
Hybotics_SoftSerial *Hybotics_SoftSerial::active_object = 0;
char Hybotics_SoftSerial::_receive_buffer[_SS_MAX_RX_BUFF]; 
volatile uint8_t Hybotics_SoftSerial::_receive_buffer_tail = 0;
volatile uint8_t Hybotics_SoftSerial::_receive_buffer_head = 0;
...
/* static */
inline void Hybotics_SoftSerial::tunedDelay(uint16_t delay) {
  uint8_t tmp=0;
...
bool Hybotics_SoftSerial::listen() {
	bool result = true;
...
void Hybotics_SoftSerial::tx_pin_write(uint8_t pin_state) {
  if (pin_state == LOW) {
...
uint8_t Hybotics_SoftSerial::rx_pin_read() {
...
/* static */
inline void Hybotics_SoftSerial::handle_interrupt() {
  if (active_object) {
    active_object->recv();
  }
}
...
ISR(PCINT0_vect) {
  Hybotics_SoftSerial::handle_interrupt();
}
#endif
...
// Constructor
//
Hybotics_SoftSerial::Hybotics_SoftSerial (uint8_t receivePin, uint8_t transmitPin, bool inverse_logic /* = false */) :
...
// Destructor
//
Hybotics_SoftSerial::~Hybotics_SoftSerial() {
  end();
}

void Hybotics_SoftSerial::setTX(uint8_t tx) {
  pinMode(tx, OUTPUT);
...
void Hybotics_SoftSerial::setRX(uint8_t rx) {
  pinMode(rx, INPUT);
...
void Hybotics_SoftSerial::begin(long speed) {
