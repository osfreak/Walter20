...
*/
/**************************************************************************/
Hybotics_10DOF_Unified::Hybotics_10DOF_Unified (void) {
}
...
*/
/**************************************************************************/
bool Hybotics_10DOF_Unified::begin() {
	//	Enable I2C
	Wire.begin();
...
*/
/**************************************************************************/
bool Hybotics_10DOF_Unified::accelGetOrientation (sensors_event_t *event, sensors_vec_t *orientation) {
	//	Make sure the input is valid, not null, etc. */
	if ((event == NULL) || (orientation == NULL)) {
		return false;
	}
	...
	/*
		...
		where:  x, y, and z are returned values from the accelerometer sensor
	*/
	...
	/*
		...
		where:  x, y, and z are returned values from accelerometer sensor
	*/
...
*/
/**************************************************************************/
bool Hybotics_10DOF_Unified::magTiltCompensation (sensors_axis_t axis, sensors_event_t *mag_event, sensors_event_t *accel_event) {
	//	Make sure the input is valid, not null, etc.
	if ((mag_event == NULL) || (accel_event == NULL)) {
		return false;
	}
	...
	float t_roll, rollRadians;
	float t_pitch, pitchRadians;
	...
	rollRadians = (float)atan2(accel_Y, sqrt(t_roll));

	t_pitch = accel_Y * accel_Y + accel_Z * accel_Z;
	pitchRadians = (float)atan2(accel_X, sqrt(t_pitch));

	cosRoll = (float)cos(rollRadians);
	sinRoll = (float)sin(rollRadians);
	cosPitch = (float)cos(pitchRadians);
	sinPitch = (float)sin(pitchRadians);
...
*/
/**************************************************************************/
bool Hybotics_10DOF_Unified::magGetOrientation (sensors_axis_t axis, sensors_event_t *event, sensors_vec_t *orientation) {
	//	Make sure the input is valid, not null, etc.
	if ((event == NULL) || (orientation == NULL)) {
		return false;
	}
